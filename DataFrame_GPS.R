...
#interval in seconds (5 min)
inter <- 300

#load data for male and female individual
load("Male.RData")
load("Female.RData")
...
time <- vector()
dist <- vector()
#headings, directions towards the other
head_A <- vector()
head_B <- vector()
head_AB <- vector()
head_BA <- vector()
...
for (i in beg:end){
  ...
  if (isTRUE(dates[j] - dates[i] == inter)){
    ...
    if(is.na(lon_A) | is.na(lat_A) | is.na(lon_B) | is.na(lat_B) |
       is.na(lon_A1) | is.na(lat_A1) | is.na(lon_B1) | is.na(lat_B1)){
    }else{
      #store all values
      #positions, time, distance
      xA <- c(xA, lon_A)
      ...
      #bearings in the interval [0,360)
      if (b1 < 0){
        b1 <- 360 + b1
      }
      if (b2 < 0){
...
#dataframe storing all the data
